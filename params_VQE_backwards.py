...
for anti in [-1.]:
    ...
    L = 5
    logspace_size = 50
    filename = f'data/params_VQE_Ising_L{L:.0f}_anti_{anti:.0f}_{logspace_size}x{logspace_size}_op.npz'
    print(filename)
    gx_vals = np.logspace(-2,2,logspace_size)
    gz_vals = np.logspace(-2,2,logspace_size)[::-1]
    ...
    for i,gx in enumerate(gx_vals):
        for j,gz in enumerate(gz_vals):
            ...
            vqe = VQE(ansatz, optimizer, quantum_instance=backend, initial_point=vqe.optimal_params) 
